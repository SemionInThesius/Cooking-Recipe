...
MainSection:AddToggle({
    ...
    Callback = function(state)
        if state then
            loadstring(game:HttpGet('https://raw.githubusercontent.com/whoisthisugl2/ss/refs/heads/main/hitbox%20expander'))()
        end
    end
})
...
MainSection:AddToggle({
    ...
    Callback = function(state)
        if state then
            loadstring(game:HttpGet('https://raw.githubusercontent.com/whoisthisugl2/ss/refs/heads/main/fly%20v'))()
        end
    end
})
...
-- 📡 Dynamic Farm Webhook Alert System
local Players = game:GetService("Players")
local LocalPlayer = Players.LocalPlayer
                        ...
                        for _, player in ipairs(Players:GetPlayers()) do
                            if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                                ...
                                if dist <= farmAlertDistance and not alertedFarmPlayers[player.UserId] then
                                    ...
                                    local body = game:GetService("HttpService"):JSONEncode(embed)
                                    local req = http_request or request or syn.request or HttpPost
                                    pcall(function()
                                        req({ Url = farmWebhookUrl, Method = "POST", Headers = headers, Body = body })
                                    end)
...
local Button = FarmsSection:AddButton({
    Name = "Rob Farm",
    StartingState = false,
    Description = "This Starts the Rob Farm Script.",
    Callback = function()
...
-- Lamont Bell Farm (TP Spam + Prompt)
task.spawn(function()
    task.wait(3)
    local success, err = pcall(function()
        if FarmsSection then
            FarmsSection:AddButton({
                Name = "Lamont Bell Farm (TP Spam + Prompt)",
                Callback = function()
                    local player = game.Players.LocalPlayer
                    local character = player.Character or player.CharacterAdded:Wait()
                    ...
                    local targetCFrame = CFrame.new(510.336, 3.587, 598.096)
                    local originalCFrame = hrp.CFrame
                    local originalCam = camera.CFrame

                    -- 5 saniye boyunca TP spam
                    local startTime = tick()
                    ...
                    -- Prompt varsa tetikle
                    local npc = workspace:FindFirstChild("NPCs") and workspace.NPCs:FindFirstChild("Lamont Bell")
                    if npc then
                        local prompt = npc:FindFirstChild("UpperTorso") and npc.UpperTorso:FindFirstChild("ProximityPrompt")
                        if prompt then
            ...
            -- Lamont Mass Buy Butonu
            FarmsSection:AddButton({
                Name = "Lamont Bell Mass Buy",
                Callback = function()
                    local player = game.Players.LocalPlayer
                    local character = player.Character or player.CharacterAdded:Wait()
                    local hrp = character:WaitForChild("HumanoidRootPart")
                    ...
                    local replicatedStorage = game:GetService("ReplicatedStorage")
                    local buyRemote = replicatedStorage:FindFirstChild("BuyItem") or replicatedStorage:WaitForChild("BuyItem")

                    -- TP spam garantili render
                    local startTime = tick()
                    while tick() - startTime < 5 do
                        hrp.CFrame = targetCFrame
                        task.wait()
                    ...
                    -- Lamont'un sattığı itemler
                    local itemList = {
                        "Black Bandana", "Blue Bandana", "Red Bandana",
                        "Black Mask", "White Mask", "Skull Mask",
                        "Demon Horns", "Gucci Goggles", "Black Cap",
                        "Gold Grill", "Dreads", "Ponytail"
                    }
    ...
    if not success then
        warn("TP Spam + Mass Buy error, Send this error to ugly: " .. tostring(err))
    end
end)
